...
#endif
#include <linux/timer.h> // timer_list, etc.
...
static char msg[200];
// Used to store the number of times the timer has executed
static u_int8_t timer_calls = 0;
// define a interrupt wait queue
...
void my_timer_callback( struct timer_list *t )
{
	...
	printk(KERN_INFO "%s\n", __func__);
	timer_calls++;
	my_timer.expires = jiffies + (1*HZ); // 1 sec.
	add_timer(&my_timer);
...
static ssize_t device_write(struct file *filp, const char __user *buffer, size_t len, loff_t *off)
{
	...
	// add an extra char at the end of the string. The char is produced and incremented from the timer.
	msg[len-1] = timer_calls;
	msg[len] = '\0';
	return len;
}
...
static int __init cdevexample_module_init(void)
{
	...
	*/
	...
	if (ret < 0){
  	...
  	/* setup the timer to call my_timer_callback when timer expires */
